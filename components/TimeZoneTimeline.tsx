    ...
    return Array.from({ length: 24 }).map((_, i) => {
      ...
      return (
        <div
          ...
          style={{ borderLeft: i > 0 ? '2px solid #e5e7eb' : 'none' }}
        />
      );
    });
  };
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-4">
        {isEditing ? (
          <input
            ...
            className="text-4xl font-bold text-center w-40 border-b-2 border-blue-500 focus:outline-none"
            autoFocus
          />
        ) : (
      ...
      >
        ...
        <div
          className="absolute top-0 bottom-0 flex items-center justify-center w-0.5 bg-blue-600 z-10 transition-all duration-200 cursor-move"
          style={{
            left: `${getCurrentTimePosition()}%`,
            transition: isDragging ? 'none' : 'left 0.2s ease-out',
          }}
        >
          <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 bg-blue-600 rounded-lg p-0.5 py-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <GripHorizontal size={16} className="text-white" />
          </div>
        </div>
        
        <div className="absolute -bottom-5 left-0 right-0 w-full h-6">
          {getHourLabels()}
        </div>
      </div>
    </div>
